...
function obtenerMunicipios($estado = null) {
    global $conn;
    try {
        if ($estado) {
            $sql = "SELECT id, nombre_municipio FROM municipios WHERE estado = ? ORDER BY nombre_municipio ASC";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$estado]);
        } else {
            $sql = "SELECT id, nombre_municipio FROM municipios ORDER BY nombre_municipio ASC";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
        }
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log("Error en obtenerMunicipios: " . $e->getMessage());
        return [];
    }
}
...
function guardarPropiedad($data, $usuario_id) {
    ...
    try {
        ...
        if ($municipio) {
            try {
                $stmt_mun = $conn->prepare("SELECT m.nombre AS municipio_nombre, e.nombre AS estado_nombre 
                                            FROM municipios m 
                                            LEFT JOIN estados e ON m.estado_id = e.id 
                                            WHERE m.id = ?");
                $stmt_mun->execute([$municipio]);
                $info = $stmt_mun->fetch(PDO::FETCH_ASSOC);
            } catch (PDOException $e) {
                $e->getMessage();
            }
        }
        ...
        $params_details = [
            ...
            !empty($data['banos']) ? (float)$data['banos'] : 0,
        ...
        foreach ($servicios as $tipo => $estado_srv) {
            $stmt_services->execute([$property_id, $tipo, (int)$estado_srv['activo'], (int)$estado_srv['adeudo']]);
        }
